...
app.post("/api/personas", async (req, res) => {
  ...
  // La edad es obligatoria y debe ser un entero mayor o igual a 0.
  if (
    age === undefined ||
    age === null ||
    String(age).trim() === "" ||
    ...
    return res.status(400).json({ error: "La edad es obligatoria y debe ser valida" });
  }
...
app.delete("/api/personas/:id", async (req, res) => {
  const id = parseId(req.params.id);
  if (!id) return res.status(400).json({ error: "El id no es valido" });
  try {
    await prisma.person.delete({ where: { id } });
    res.status(204).end(); // sin body
  } catch (e) {
    if ((e as { code?: string }).code === "P2025") return res.status(404).json({ error: "No existe" });
